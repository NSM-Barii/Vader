...
class Mass_IP_Scanner:
                        ...
                        if cls.save and cls.current_ips:
                            with LOCK:
                                File_Saver.push_ips_found(
                                    data=cls.current_ips, CONSOLE=console, verbose=True
                                )
            ...
            if cls.country:
                ...
                with LOCK:
                    if cls.bf_all is None:
                        cls.bf_all = BloomFilter(
                            capacity=cls.bloom_size, error_rate=0.001
                        )

                    random_ip = f"{random.randint(0, 255)}.{random.randint(0, 255)}.{random.randint(0, 255)}.{random.randint(0, 255)}"

                    if random_ip in cls.bf_all:
                        return False
                    cls.bf_all.add(random_ip)
                    cls.scanned_ips += 1
    ...
    def _ip_threader(cls, ports, panel, max_workers=250, timeout=1):
        ...
        try:
            portz = [int(port) for port in ports.split(",")]
        except Exception:
            portz = list(ports)
            ...
            try:
                while cls.scan:
                    ...
                    if time.time() - last_save > 5 and cls.save and cls.current_ips:
                        with LOCK:
                            File_Saver.push_ips_found(
                                data=cls.current_ips, CONSOLE=console, verbose=False
                            )
                            last_save = time.time()
                            cls.current_ips = []
                ...
                console.print("[bold red][-] Killing ALL Threads....")
                cls.scan = False
    ...
    def _main(cls, port, threads):
        ...
        if not port:
            ...
            threads = console.input("[bold yellow]Enter Thread count!: ") or 250
            print("\n")

        cls.time_start = time.time()
        panel = Panel(
            renderable="[bold red]Mass IP Scanner",
